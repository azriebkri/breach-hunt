...
classes. The error handler discriminates via type-predicate functions
...
any type on parameter

src/usecases/searchJobs/searchJobsInteractor.ts
...
as casting on req.body

src/application/jobApplications/applyToJobController.ts
...
HTTP error thrown in usecase layer

src/usecases/{getJob,updateJob,deleteJob}/\*Interactor.ts
...
src/application/jobs/listJobsController.ts
...
Concrete infrastructure type in usecase layer

src/usecases/applyToJob/applyToJobInteractor.ts
...
.then() chain instead of async/await

src/infrastructure/notifications/notificationClient.ts
...
src/usecases/formatJob/formatJobForPlatform.ts
...
Missing Zod validation on update endpoint

src/application/jobs/updateJobController.ts
...
Third-party type (AxiosResponse) in domain gateway

src/entities/gateways/notificationGateway.ts
...
(not re-injected in current tree)
...
Infrastructure imports API schema

src/infrastructure/jobs/inMemoryJobRepository.ts
...
Controller bypasses usecase, calls repo directly

src/application/jobs/{getJobsByCompany,getFeaturedJobs}Controller.ts
...
Domain error extends API HttpError

src/entities/errors/jobNotFoundError.ts
...
Usecase accepts Express Request
...
Usecase instantiates concrete notification client
...
Direct axios call from usecase layer

src/usecases/auditJobEvent/auditJobEventInteractor.ts
...
console.log instead of a Logger gateway

src/usecases/createJob/createJobInteractor.ts, src/entities/job.ts
...
API-layer Zod schema imported into usecase

src/usecases/createJob/createJobInteractor.ts
...
Business logic inside repository (and on gateway)

src/infrastructure/jobs/inMemoryJobRepository.ts, src/entities/gateways/jobRepository.ts
...
Switch-on-platform in formatter
...
Growing predicate chain in error handler

src/application/middleware/errorHandlerMiddleware.ts
...
Throttled notification client breaks gateway contract

src/infrastructure/notifications/throttledNotificationClient.ts
...
Read-only job repository throws on save/update/remove

src/infrastructure/jobs/readOnlyJobRepository.ts
...
Fat JobRepository gateway (persistence + reporting + notify)

src/entities/gateways/jobRepository.ts
...
Fat JobApplicationRepository gateway (CRUD + lifecycle + export)

src/entities/gateways/jobApplicationRepository.ts
...
Missing Clock gateway; new Date() scattered

src/entities/job.ts, src/usecases/applyToJob/applyToJobInteractor.ts, src/usecases/auditJobEvent/auditJobEventInteractor.ts
...
Usecase reads process.env.NOTIFICATION_ENABLED / NOTIFICATION_RETRIES
...
Domain throws generic new Error for salary breach
...
src/usecases/applyToJob/applyToJobInteractor.ts, src/usecases/{createJob,updateJob,deleteJob}/\*Interactor.ts
...
Anemic Job entity (pure data, zero behaviour)
...
Primitive obsession (salary: number, applicantEmail: string)

src/entities/job.ts, src/entities/jobApplication.ts
...
Domain model imports from infrastructure
...
DDD — anemic / primitive obsession
...
SalaryLimitExceededError: (e, r) => { /_ ... _/ },
...
src/usecases/applyToJob/applyToJobInteractor.ts — try { await legacyNotifier.send(...) } catch (\_err) { /_ ignored _/ } and catch (\_err) { attempt += 1; } on the retry loop.
...
Found almost every violation and proposed correct fixes.

"Senior"

42–51 (≥ 65%)

Found most layering, DIP, and SOLID violations.

"Mid"

32–41 (≥ 50%)

Found code-style + surface-level architecture issues.

"Junior"

< 32

Reread the README and run through docs/clean-architecture.md.
